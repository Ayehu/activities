...
Namespace Ayehu.Sdk.ActivityCreation
    Public Class ActivityClass
        ...
        Public Function Execute() As ICustomActivityResult Implements IActivity.Execute
            ...
            dt.Columns.Add("Memory", GetType(Integer))
            ...
            If ProcessName.StartsWith("*") OrElse ProcessName.EndsWith("*") Then
                ...
                HasWildCard = True
            End If
            Dim oQuery As ObjectQuery = New System.Management.ObjectQuery("Select Name, IDProcess, WorkingSetPrivate from  Win32_PerfFormattedData_PerfProc_Process where name LIKE '%" & ProcessName & "%'")
            ...
            For Each oReturn As ManagementObject In oSearcher.Get()
                ...
                If HasWildCard = True Then
                    Fnd = True
                    dt.Rows.Add(oReturn("IDProcess"), Math.Round(val(oReturn("WorkingSet")) / 1024, 2))
                Else
                    If LCase(oReturn("Name")) = LCase(ProcessName) OrElse LCase(oReturn("Name")) = LCase(ProcessName & ".exe") OrElse LCase(oReturn("Name")).Contains(LCase(ProcessName) & "#") OrElse LCase(oReturn("Name")).Contains(LCase(ProcessName) & ".exe#") Then
                        Fnd = True
                        dt.Rows.Add(oReturn("IDProcess"), Math.Round(val(oReturn("WorkingSetPrivate")) / 1024, 2))
                    End If
                End If
